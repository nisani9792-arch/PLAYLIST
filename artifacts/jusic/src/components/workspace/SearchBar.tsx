...
import { Search, Loader2, Music, Plus, Tag } from 'lucide-react';
...
export function SearchBar({ onAddSong }: { onAddSong: (song: MsHit) => void }) {
  ...
  useEffect(() => {
    ...
    const search = async () => {
      setIsSearching(true);
      try {
        const hits = await meilisearchSearch(debouncedQuery, 20);
        setResults(hits);
      ...
      <AnimatePresence>
        {showDropdown && results.length > 0 && (
          <motion.div
            ...
            className="absolute top-full left-0 right-0 mt-2 border border-black/[0.07] rounded-2xl overflow-hidden max-h-[30rem] overflow-y-auto"
            style={{
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="flex flex-col min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground flex items-center gap-2 truncate">
                        ...
                        {hit.album && <><span className="opacity-40">·</span><span className="truncate opacity-70">{hit.album}</span></>}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {hit.genre && (
                      <Badge variant="secondary" className="hidden sm:inline-flex bg-primary/8 text-primary border border-primary/15 rounded-lg text-[10px]">
                        {hit.genre}
                      </Badge>
                    )}
                    <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                      <Button size="sm" className="rounded-xl h-7 px-3 text-xs shadow-sm" onClick={() => handleAdd(hit)}>
