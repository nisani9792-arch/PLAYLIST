...
export default function Workspace({ operatorName, offline = false }: WorkspaceProps) {
  ...
  return (
    <div className="app-shell-bg flex flex-col h-[100dvh] w-full overflow-hidden text-foreground selection:bg-primary/20 selection:text-foreground">
      <header className="bp-glass-strip flex-shrink-0 flex flex-col z-40 pt-[max(env(safe-area-inset-top,0px),0.625rem)] sm:pt-3.5 bg-card/90">
        <div className="flex flex-wrap md:flex-nowrap items-center justify-between gap-2 sm:gap-3 px-3 sm:px-5 lg:px-7 py-2 sm:py-3.5">
          ...
          <div className="flex flex-wrap md:flex-nowrap items-center justify-end gap-1 sm:gap-1 shrink-0 rounded-2xl md:rounded-none border md:border-transparent border-border/50 bg-muted/40 md:bg-transparent px-1 py-1">
            <ApiStatusIndicator />
            <InstallAppButton />
        ...
        <div className="px-3 sm:px-5 lg:px-7 pb-2.5 sm:pb-3 md:hidden relative z-[80]">
          <SearchBar onAddSong={playlist.addSong} />
        </div>
        <div className="hidden md:block px-5 lg:px-7 pb-3 relative z-[80] max-w-5xl mx-auto w-full">
          <SearchBar onAddSong={playlist.addSong} />
        </div>
      </header>

      <main className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0 p-0 md:p-2.5 pb-[max(env(safe-area-inset-bottom,0px),0.5rem)] relative z-10 isolate">
        <ASIComposerPanel
